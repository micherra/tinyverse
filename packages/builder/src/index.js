...
const addDiagnostic = (list, severity, code, message, location) => {
    list.push({ severity, code, message, location });
};
const loadToolManifest = async (outDir, diags) => {
    ...
    try {
        ...
        addDiagnostic(diags, "error", "TV_DIAG_TOOL_MANIFEST_INVALID", `Failed to parse ${manifestPath}: ${String(err)}`, manifestPath);
        return null;
    }
};
const collectAssets = async (root, distRoot) => {
...
export const buildApps = async (config, options = {}) => {
    const diagnostics = [];
    ...
    for (const resource of config.appResources) {
        if (seenResources.has(resource.resourceUri)) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_UI_URI_DUPLICATE", `Duplicate resourceUri ${resource.resourceUri} in config`);
            continue;
        }
        seenResources.add(resource.resourceUri);
        if (seenTools.has(resource.toolId)) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_TOOL_UI_DUPLICATE", `Tool ${resource.toolId} is mapped to multiple resources`, config.outDir);
        }
        seenTools.add(resource.toolId);
        const toolEntry = toolLookup.get(resource.toolId);
        if (!toolEntry) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_TOOL_ID_MISSING", `Tool ${resource.toolId} from config not found in tool.manifest.json`, path.join(config.outDir, "tool.manifest.json"));
        }
        else if (toolEntry.resourceUri && toolEntry.resourceUri !== resource.resourceUri) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_TOOL_UI_MISMATCH", `Tool ${resource.toolId} is mapped to ${resource.resourceUri} in config but ${toolEntry.resourceUri} in tool.manifest.json`, path.join(config.outDir, "tool.manifest.json"));
        }
        if (!(await fs.pathExists(resource.entry))) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_BUILD_ENTRYFILE_MISSING", `Entry file missing for ${resource.resourceUri}: ${resource.entry}`);
            continue;
        }
        const parsed = parseResourceUri(resource.resourceUri);
        if (!parsed) {
            addDiagnostic(diagnostics, "error", "TV_DIAG_UI_URI_INVALID", `Invalid resourceUri ${resource.resourceUri} (expected ui://namespace/resource)`);
            continue;
        }
        ...
        try {
            ...
            addDiagnostic(diagnostics, "error", "TV_DIAG_BUILD_FAILED", `Vite build failed for ${resource.resourceUri}: ${String(err)}`);
            continue;
        }
    ...
    if (toolManifest) {
        ...
        for (const tool of toolManifest.tools) {
            if (tool.resourceUri && !resourceUriSet.has(tool.resourceUri)) {
                addDiagnostic(diagnostics, "error", "TV_DIAG_UI_URI_MISSING_IN_APPS_MANIFEST", `Tool ${tool.id} references ${tool.resourceUri} but it is missing from config/appResources`, manifestPath);
            }
        }
    }
