  ...
  for (const resource of resourcesToBuild) {
    ...
    if (isFullApp) {
      ...
      const wrapperContent = [
        ...
        "  const [data, setData] = useState<{ data: unknown; toolId: string; resourceUri: string } | null>(null);",
        "  const [error, setError] = useState<string | null>(null);",
        "",
        "  useEffect(() => {",
        "    if (msg && msg !== data) {",
        "      setData(msg as { data: unknown; toolId: string; resourceUri: string });",
        "      return;",
        "    }",
        "    if (data || msg) return;",
        "    let aborted = false;",
        ...
        "        .then(json => {",
        "          if (!aborted && !data && !msg) {",
        `            setData({ data: (json as { result: unknown }).result, toolId: "${resource.toolId}", resourceUri: "${resource.resourceUri}" });`,
        "          }",
        "        })",
        "        .catch(err => {",
        "          if (!aborted) {",
